...
describe('RemoteEntitySelector', () => {
  ...
  const adapter: EntityLookupAdapter<Item> = {
    ...
    resolve: (id) => of({ id, label: `Resolved ${id}` }),
    id: (value) => value.id,
  ...
  it('separates loading, empty, failure and retry; clamps pages; resets stored failure on language change', () => {
    ...
    requests[2].result.next({ items: [], page: 4, pageSize: 5, totalElements: 7 });
  ...
  it('guards asynchronous initialization and known-ID resolution and renders inline controls', async () => {
    const { fixture, component } = setup();
    component.trustInitialValue({ id: 'initial', label: 'Initial' });
    type(component, 'new');
    await Promise.resolve();
    expect(component.value()).toBeNull();
    component.resolveKnownId('1');
    expect(component.value()?.label).toBe('Resolved 1');
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('[role="combobox"]')).toBeNull();
    expect(fixture.nativeElement.querySelector('.selected')?.textContent).toContain('Resolved 1');
    component.reset();
