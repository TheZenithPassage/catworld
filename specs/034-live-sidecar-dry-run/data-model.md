...
- `prepared_handoff_identity`: canonical `sidecar-prepared-handoff-v1` SHA-256 identity over the exact ordered pre-evidence run/issues, coordinator and child Git context, control revision, prepared artifact paths, dependency layer/list, PR contract, preparation/launch states, and false permissions. PowerShell ordered JSON is the canonical serialization. Artifact content is validated separately; artifact blob/content hashes, the fingerprint itself, child identity, evidence SHAs, and recording/activation heads are excluded to avoid recursion.
