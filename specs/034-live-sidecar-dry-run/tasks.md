...
- [X] T021 [TO2] Remove only the temporary managed Spec Kit pointer if required; commit and normally push immutable control-plane correction `C`; fetch and verify remote equality; then use one bounded report-only commit to store exact `C` and designate `C` as the workflow source SHA consumed by every runtime handoff, without opening the final #260 PR
