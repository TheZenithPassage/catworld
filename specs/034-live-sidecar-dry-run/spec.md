...
- A held child dispatch is rejected, ambiguous, loses its stable task identity, cannot receive targeted continuation, mutates during read-only preflight, or appears capable of implementation before release.
...
- **TR-023**: A successful held dispatch MUST return one stable child/task identity that remains the same logical child through targeted release. The deterministic prepared-handoff fingerprint MUST use the canonical `sidecar-prepared-handoff-v1` ordered JSON/SHA-256 schema over only immutable identity/context/path/PR/state inputs available before the handoff-ready evidence commit. Artifact content MUST be validated separately; artifact hashes, the fingerprint itself, child identity, and later evidence/recording fields MUST NOT be fingerprint inputs. During held preflight and until the targeted durable-continuation phase begins, that child MAY perform only approved read-only checks and MUST NOT edit, stage, execute prepared tasks, commit, push, open/update a PR, or mutate GitHub state.
- **TR-024**: Only after unambiguous dispatch acceptance MAY the coordinator record factual `launched`. The factual launched evidence MUST be committed and normally pushed; after its SHA exists, a later bounded activation/recording commit MUST store that exact SHA, be pushed normally, and pass current-remote equality plus evidence-ancestry proof. Targeted durable continuation MAY perform only the required clean fetch and normal fast-forward/merge incorporation while implementation/delivery permissions remain false; the same held child MUST verify the activation head contains the launched evidence and acknowledge release before those permissions become effective. Rejected or ambiguous dispatch, failed launch-state or recording push, failed refresh/verification, and failed release MUST preserve the approved failure semantics and perform no unauthorized child work.
...
- **SC-002**: Two independent first-layer child PRs are opened ready against the remote coordinator branch after evidence proves handoff-ready evidence plus its containing recording head before dispatch, stable held identities, zero held-preflight mutations, factual launched evidence plus its containing activation head before targeted continuation, barrier-only activation-head incorporation with permissions still false, and launched-evidence ancestry verification before release; each PR records its issue, worktree, commit, URL, target, exactly two `Related to` references, and fresh passing validation before Mandatory Pause 1.
