...
# Resolve the tasks template from the repository's core templates
...
if (-not $tasksTemplate -or -not (Test-Path -LiteralPath $tasksTemplate -PathType Leaf)) {
    ...
    [Console]::Error.WriteLine("ERROR: Core tasks template not found for repository root: $($paths.REPO_ROOT)`nExpected core template location: $expectedCoreTemplate`nTo continue, restore '.specify/templates/tasks-template.md' (for example by re-running 'specify init').")
    exit 1
}
